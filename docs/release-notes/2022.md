...
#### What's new
- A user will be able to create an account using Organization email. Please note that private email domains, such as gmail & yahoo, are not supported. Once a user account is created, the user will be able to login with One-Time Passcode (OTP) send to the organization email on file, update account settings, choose product areas, save filter preferences, receive documentation updates and notifications, setup security questions, download user data, and delete an account.
- A forgotten password can be recovered with account email and One-Time Passcode (OTP) or preset security questions.
- Additionally, a loogged-in user will be able to create workspaces for supported Fiserv Products to manage specific Fiserv Product integration and credentials.
- Some of the Internal Fiserv Products will be visible only to logged-in users with Fiserv emails.
