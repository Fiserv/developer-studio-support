...
#### What's new
- A user will be able to [register an account](?path=docs/registration.md) to save filter preferences, receive documentation updates and notifications and create workspaces.
- Additionally, a loogged-in user will be able to create [workspace](?path=docs/guides/workspaces.md) for supported Fiserv Products to manage specific Fiserv Product integration and credentials.
